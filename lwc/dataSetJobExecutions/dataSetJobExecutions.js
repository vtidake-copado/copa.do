...
import getJobExecutionIds from '@salesforce/apex/DataSetJobExecutionsCtrl.getJobExecutionIds';
...
export default class DataSetJobExecutions extends LightningElement {
    ...
    async _fetchColumnConfigurations() {
        try {
            ...
            data.forEach(column => {
                if (column.fieldName == 'CreatedDate') {
    ...
    async _fetchDataRows() {
        try {
            const jobExecutionIds = await getJobExecutionIds({ dataSetId: this.recordId });
