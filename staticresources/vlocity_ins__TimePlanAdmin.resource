...
angular.module('timeplanadmin', ['vlocity', 'ngSanitize', 'cpqdirectives', 'sldsangular'])
.config(['remoteActionsProvider', function(remoteActionsProvider) {
    remoteActionsProvider.setRemoteActions(window.remoteActions || {});
}])
.config(['$locationProvider', function($locationProvider) {
...
require('./modules/timeplanadmin/controller/TimePlanAdminController.js');

},{"./CpqDirectives.js":1,"./modules/timeplanadmin/controller/TimePlanAdminController.js":35}],3:[function(require,module,exports){
...
angular.module('timeplanadmin')
.controller('TimePlanAdminController', ['$scope', '$rootScope', '$location', '$timeout', '$compile', 'remoteActions',
    function ($scope, $rootScope, $location, $timeout, $compile, remoteActions) {
        $scope.OBJECT_NAME = 'TimePlan__c';
        $scope.nsp = fileNsPrefix();
        $scope.timeplanId = $location.search().id;
        $scope.timeplanName = $location.search().name;
        ...
        $scope.getObject = function(sectionId) {
            remoteActions.getObject(sectionId).then(function(results) {
                $scope.timeplan = results;
                $scope.editTimeplan = {};
                for (var key in $scope.timeplan) {
                    $scope.editTimeplan[key] = $scope.timeplan[key];
                }
            });
        };
        ...
        $scope.createItem = function(event) {
            ...
            var itemToSave = {};
            for (var key in $scope.editTimeplan) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = $scope.editTimeplan[key];
                }
            }

            remoteActions.createObject($scope.nsp + $scope.OBJECT_NAME, itemToSave).then(function(results) {
                for (var key in results) {
                    $scope.timeplan[key] = results[key];
                    $scope.editTimeplan[key] = results[key];
                }
                if (event) {
                    event.currentTarget.innerText = originalText;
                    event.currentTarget.disabled = false;
                ...
                window.location = '/apex/TimePlanAdmin?id=' + results.Id + '&name=' + results.Name;
            }, function(error) {
                //TODO: show save failure message
                if (event) {
                    event.currentTarget.innerText = 'Error!';
        ...
        $scope.saveItem = function(event) {
            ...
            for (var key in $scope.editTimeplan) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = ($scope.editTimeplan[key] === null ? undefined : $scope.editTimeplan[key]);
                }
            }
            ...
            remoteActions.updateObject(itemToSave).then(function(results) {
                ...
                for (var key in results) {
                    if ($scope.objectFields[key] && $scope.objectFields[key].type === 'DATE') {
                        var tzOffset = (window.userTimezoneOffset || window.parent.userTimezoneOffset);
                        $scope.timeplan[key] = results[key] + tzOffset;
                        $scope.editTimeplan[key] = results[key] + tzOffset;
                    } else {
                        $scope.timeplan[key] = results[key];
                        $scope.editTimeplan[key] = results[key];
                    }
        ...
        $scope.init = function() {
            ...
            if ($scope.mode === 'new') {
                $scope.timeplanName = 'New Time Plan';
                $scope.getObjectLayoutByName($scope.OBJECT_NAME, '');
            } else {
                $scope.getObjectLayoutById($scope.timeplanId, true);
                $scope.getObject($scope.timeplanId);
            }
        };
        $scope.init();
    }
