...
},{"./CpqDirectives.js":1,"./modules/timepolicyadmin/controller/TimePolicyAdminController.js":35}],3:[function(require,module,exports){
...
.controller('TimePolicyAdminController', ['$scope', '$rootScope', '$location', '$timeout', '$compile', 'remoteActions',
    function ($scope, $rootScope, $location, $timeout, $compile, remoteActions) {
        ...
        $scope.getObject = function(sectionId) {
            remoteActions.getObject(sectionId).then(function(results) {
                ...
                for (var key in $scope.timepolicy) {
                    $scope.editTimepolicy[key] = $scope.timepolicy[key];
                }
            });
        };
        ...
        $scope.createItem = function(event) {
            ...
            var itemToSave = {};
            for (var key in $scope.editTimepolicy) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = $scope.editTimepolicy[key];
                }
            }
            ...
            remoteActions.createObject($scope.nsp + $scope.OBJECT_NAME, itemToSave).then(function(results) {
                for (var key in results) {
                    ...
                    $scope.editTimepolicy[key] = results[key];
                }
                if (event) {
                    event.currentTarget.innerText = originalText;
                    event.currentTarget.disabled = false;
                ...
                window.location = '/apex/TimePolicyAdmin?id=' + results.Id + '&name=' + results.Name;
            }, function(error) {
                //TODO: show save failure message
                if (event) {
                    event.currentTarget.innerText = 'Error!';
        ...
        $scope.saveItem = function(event) {
            ...
            for (var key in $scope.editTimepolicy) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = ($scope.editTimepolicy[key] === null ? undefined : $scope.editTimepolicy[key]);
                }
            }
            ...
            remoteActions.updateObject(itemToSave).then(function(results) {
                ...
                for (var key in results) {
                    if ($scope.objectFields[key] && $scope.objectFields[key].type === 'DATE') {
                        ...
                        $scope.editTimepolicy[key] = results[key] + tzOffset;
                    } else {
                        $scope.timepolicy[key] = results[key];
                        $scope.editTimepolicy[key] = results[key];
                    }
