...
},{"./modules/cpqdirectives/controller/NewObjectRecordController.js":3,"./modules/cpqdirectives/directive/VlocAssignAttrsFields.js":4,"./modules/cpqdirectives/directive/VlocAttachment.js":5,"./modules/cpqdirectives/directive/VlocAttachments.js":6,"./modules/cpqdirectives/directive/VlocAttribute.js":7,"./modules/cpqdirectives/directive/VlocAttributeMetadata.js":8,"./modules/cpqdirectives/directive/VlocCustomView.js":9,"./modules/cpqdirectives/directive/VlocDataTable.js":10,"./modules/cpqdirectives/directive/VlocFieldMetadata.js":11,"./modules/cpqdirectives/directive/VlocImgCarousel.js":12,"./modules/cpqdirectives/directive/VlocLayoutElement.js":13,"./modules/cpqdirectives/directive/VlocLayoutManagement.js":14,"./modules/cpqdirectives/directive/VlocObjAttrsFields.js":15,"./modules/cpqdirectives/directive/VlocObjField.js":16,"./modules/cpqdirectives/directive/VlocOfferPricingComponent.js":17,"./modules/cpqdirectives/factory/CpqService.js":18,"./modules/cpqdirectives/templates/templates.js":19}],2:[function(require,module,exports){
angular.module('pricingadmin', ['vlocity', 'ngSanitize', 'cpqdirectives', 'sldsangular'])
.config(['remoteActionsProvider', function(remoteActionsProvider) {
    remoteActionsProvider.setRemoteActions(window.remoteActions || {});
}])
.config(['$locationProvider', function($locationProvider) {
...
require('./modules/pricingadmin/controller/PricingAdminController.js');

},{"./CpqDirectives.js":1,"./modules/pricingadmin/controller/PricingAdminController.js":20}],3:[function(require,module,exports){
...
.controller('NewObjectRecordController', ['$scope', '$location', 'remoteActions', '$timeout',
    function ($scope, $location, remoteActions, $timeout) {
        ...
        $scope.saveItem = function(event) {
            ...
            if ($scope.objectName === 'Product2') {
                $scope.createProduct(event, itemToSave);
            } else {
                $scope.createItem(event, itemToSave);
            }
        };

        $scope.createProduct = function(event, itemToSave) {
            var originalText;
            if (event) {
            ...
            var inputMap = {
                'recordTypeName': $scope.recordType,
                'productObject': itemToSave
            };
            var inputMapJSON = JSON.stringify(inputMap);

            remoteActions.createProduct(inputMapJSON).then(function(results) {
                console.log('create productClass results: ', results);
                if (event) {
...
.directive('vlocCustomView', ['$compile',
    function($compile) {
        var VIEW_TEMPLATE_MAP = {
            ...
            'ProductPricing': '<vloc-product-pricing custom-view-attrs="customViewData.attrs"></vloc-product-pricing>',
    ...
    function($rootScope, remoteActions) {
        return {
            ...
            controller: function($scope, $timeout) {
                ...
                $scope.describeObject = function(objectName) {
                    remoteActions.describeObject(objectName).then(function(results) {
                        console.log('describeObject objectfacet results: ', results);
                        $scope.objectFields = results;
                    });
                };
                ...
                $scope.saveItem = function(event) {
                    ...
                    if (itemToSave.Id === undefined) {
                        ...
                        remoteActions.updateObject(itemToSave).then(function(results) {
                            console.log('save ' + $scope.objectName + ' results: ', results);
                            $rootScope.$broadcast('refreshItems');
                            $scope.elementObj = {};
                            for (var key in results) {
                                $scope.elementObj[key] = results[key];
                ...
                $scope.init = function() {
                    console.log('objectName: ', $scope.objectName);
                    ...
                    console.log('elementObj: ', $scope.elementObj);
...
.controller('PricingAdminController', ['$scope', '$rootScope', '$location', '$timeout', '$compile', 'remoteActions',
    function ($scope, $rootScope, $location, $timeout, $compile, remoteActions) {
        ...
        $scope.getObjectLayoutByName = function(objectName, recordType) {
            remoteActions.getObjectLayoutByName(objectName, recordType).then(function(results) {
                console.log('getObjectLayoutByName pricing comp results: ', results);
                $scope.buildObjectLayout(results);
            });
        };
        ...
        $scope.getObjectLayoutById = function(objectId, forSelf) {
            remoteActions.getObjectLayoutById(objectId, forSelf).then(function(results) {
                console.log('getObjectLayoutById pricing comp results: ', results);
                $scope.buildObjectLayout(results);
            });
        };
        ...
        $scope.buildObjectLayout = function(results) {
            ...
            console.log('FACETS for pricing comp: ', $scope.facets);
        };

        $scope.describeObject = function(objectName) {
            remoteActions.describeObject(objectName).then(function(results) {
                console.log('describeObject pricing comp results: ', results);
                $scope.objectFields = results;
            });
        };
        ...
        $scope.createItem = function(event) {
            ...
            remoteActions.createObject($scope.nsp + $scope.OBJECT_NAME, itemToSave).then(function(results) {
                console.log('create pricing comp results: ', results);
                for (var key in results) {
                ...
                window.location = '/apex/PricingComponentAdmin?id=' + results.Id + '&name=' + results.Name;
            }, function(error) {
                //TODO: show save failure message
                if (event) {
                    event.currentTarget.innerText = 'Error!';
        ...
        $scope.saveItem = function(event) {
            ...
            remoteActions.updateObject(itemToSave).then(function(results) {
                console.log('save pricing comp results: ', results);
                $scope.price = {};
                $scope.editPrice = {};
                for (var key in results) {
                    if ($scope.objectFields[key] && $scope.objectFields[key].type === 'DATE') {
        ...
        $scope.init = function() {
            ...
            if ($scope.mode === 'new') {
                $scope.pricingComponentName = 'New Pricing Component';
                $scope.getObjectLayoutByName($scope.OBJECT_NAME, '');
            } else {
