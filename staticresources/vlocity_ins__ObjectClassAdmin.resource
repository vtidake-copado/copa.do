...
},{"./CpqDirectives.js":1,"./modules/objectclassadmin/controller/ObjectClassAdminController.js":35,"./modules/objectclassadmin/directive/VlocAttrFieldBindings.js":36,"./modules/objectclassadmin/directive/VlocObjectClassAttrsFields.js":37,"./modules/objectclassadmin/directive/VlocObjectTypeStructure.js":38,"./modules/objectclassadmin/factory/ObjectClassService.js":39,"./modules/objectclassadmin/templates/templates.js":40}],3:[function(require,module,exports){
...
.controller('ObjectClassAdminController', ['$scope', '$location', '$timeout', '$compile', 'remoteActions', 'objectClassService',
    function ($scope, $location, $timeout, $compile, remoteActions, objectClassService) {
        $scope.OBJECT_NAME = 'ObjectClass__c';
        $scope.nsp = fileNsPrefix();
        ...
        $scope.getObject = function(objectClassId) {
            remoteActions.getObject(objectClassId).then(function(results) {
                console.log('getObject results: ', results);
                $scope.objectClass = results;
                $scope.editObjectClass = {};
                for (var key in $scope.objectClass) {
                    $scope.editObjectClass[key] = $scope.objectClass[key];
                }
        ...
        $scope.createItem = function(event) {
            ...
            var itemToSave = {};
            for (var key in $scope.editObjectClass) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = $scope.editObjectClass[key];
                }
            }
            ...
            remoteActions.createObject($scope.nsp + $scope.OBJECT_NAME, itemToSave).then(function(results) {
                ...
                for (var key in results) {
                    ...
                    $scope.editObjectClass[key] = results[key];
                }
                if (event) {
                    event.currentTarget.innerText = originalText;
                    event.currentTarget.disabled = false;
                ...
                window.location = '/apex/objectClassAdmin?id=' + results.Id + '&name=' + results.Name;
            }, function(error) {
                //TODO: show save failure message
                if (event) {
                    event.currentTarget.innerText = 'Error!';
        ...
        $scope.saveItem = function(event) {
            ...
            for (var key in $scope.editObjectClass) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = ($scope.editObjectClass[key] === null ? undefined : $scope.editObjectClass[key]);
                }
            }
            ...
            remoteActions.updateObject(itemToSave).then(function(results) {
                ...
                for (var key in results) {
                    if ($scope.objectFields[key] && $scope.objectFields[key].type === 'DATE') {
                        ...
                        $scope.editObjectClass[key] = results[key] + tzOffset;
                    } else {
                        $scope.objectClass[key] = results[key];
                        $scope.editObjectClass[key] = results[key];
                    }
...
angular.module('objectclassadmin')
.directive('vlocAttrFieldBindings', ['$rootScope', 'remoteActions',
    function($rootScope, remoteActions) {
        return {
            scope: {
                customViewAttrs: '='
            ...
            controller: function($scope, $sldsModal) {
                ...
                $scope.newBinding = function() {
                    ...
                    modalScope.createBinding = function() {
                        remoteActions.createAttributeFieldBinding(this.editAttrId, this.editFieldName, $scope.objectId).then(function(results) {
                ...
                $scope.deleteBinding = function(bindingId, attrName, fieldLabel) {
                    ...
                    modalScope.confirmAction = function() {
                        remoteActions.deleteObject(this.bindingObj).then(function(results) {
...
angular.module('objectclassadmin')
.directive('vlocObjectClassAttrsFields', ['$rootScope', 'remoteActions',
    function($rootScope, remoteActions) {
        return {
            scope: {
                customViewAttrs: '='
...
angular.module('objectclassadmin')
.directive('vlocObjectTypeStructure', ['remoteActions',
    function(remoteActions) {
        return {
            scope: {
                customViewAttrs: '='
            ...
            controller: function($scope, $rootScope, $sldsModal, $compile) {
                ...
                $scope.getFieldSets = function() {
                    remoteActions.getFieldSetsByName($scope.nsp + 'ObjectClass__c').then(function(results) {
                        if (results[$scope.nsp + 'objecttype'] === undefined) {
                            $scope.prodFieldSet = null;
                        } else {
                            $scope.prodFieldSet = results[$scope.nsp + 'objecttype'];
                        }
                    });
                };
                ...
                $scope.getObjectTypeStructure = function() {
                    remoteActions.getObjectTypeStructure($scope.objectId).then(function(results) {
                        var objectTypeList = [];
