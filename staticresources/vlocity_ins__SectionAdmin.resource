...
angular.module('sectionadmin', ['vlocity', 'ngSanitize', 'cpqdirectives', 'sldsangular'])
.config(['remoteActionsProvider', function(remoteActionsProvider) {
    remoteActionsProvider.setRemoteActions(window.remoteActions || {});
}])
.config(['$locationProvider', function($locationProvider) {
...
require('./modules/sectionadmin/controller/SectionAdminController.js');

},{"./CpqDirectives.js":1,"./modules/sectionadmin/controller/SectionAdminController.js":35}],3:[function(require,module,exports){
...
angular.module('sectionadmin')
.controller('SectionAdminController', ['$scope', '$rootScope', '$location', '$timeout', '$compile', 'remoteActions',
    function ($scope, $rootScope, $location, $timeout, $compile, remoteActions) {
        $scope.OBJECT_NAME = 'UISection__c';
        $scope.nsp = fileNsPrefix();
        $scope.sectionId = $location.search().id;
        $scope.sectionName = $location.search().name;
        ...
        $scope.getObject = function(sectionId) {
            remoteActions.getObject(sectionId).then(function(results) {
                ...
                for (var key in $scope.section) {
                    $scope.editSection[key] = $scope.section[key];
                }
            });
        };
        ...
        $scope.createItem = function(event) {
            ...
            var itemToSave = {};
            for (var key in $scope.editSection) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = $scope.editSection[key];
                }
            }

            remoteActions.createObject($scope.nsp + $scope.OBJECT_NAME, itemToSave).then(function(results) {
                for (var key in results) {
                    $scope.section[key] = results[key];
                    $scope.editSection[key] = results[key];
                }
                if (event) {
                    event.currentTarget.innerText = originalText;
                    event.currentTarget.disabled = false;
                ...
                window.location = '/apex/SectionAdmin?id=' + results.Id + '&name=' + results.Name;
            }, function(error) {
                //TODO: show save failure message
                if (event) {
                    event.currentTarget.innerText = 'Error!';
        ...
        $scope.saveItem = function(event) {
            ...
            for (var key in $scope.editSection) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = ($scope.editSection[key] === null ? undefined : $scope.editSection[key]);
                }
            }
            ...
            remoteActions.updateObject(itemToSave).then(function(results) {
                ...
                for (var key in results) {
                    if ($scope.objectFields[key] && $scope.objectFields[key].type === 'DATE') {
                        var tzOffset = (window.userTimezoneOffset || window.parent.userTimezoneOffset);
                        $scope.section[key] = results[key] + tzOffset;
                        $scope.editSection[key] = results[key] + tzOffset;
                    } else {
                        $scope.section[key] = results[key];
                        $scope.editSection[key] = results[key];
                    }
        ...
        $scope.init = function() {
            ...
            if ($scope.mode === 'new') {
                ...
                $scope.getObjectLayoutById($scope.sectionId, true);
                $scope.getObject($scope.sectionId);
            }
        };
        $scope.init();
    }
