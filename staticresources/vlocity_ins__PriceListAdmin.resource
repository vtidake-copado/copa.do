...
require('./modules/pricelistadmin/directive/VlocPriceListEntries.js');
require('./modules/pricelistadmin/directive/VlocPriceListHierarchy.js');
require('./modules/pricelistadmin/templates/templates.js');

},{"./CpqDirectives.js":1,"./modules/pricelistadmin/controller/PriceListAdminController.js":35,"./modules/pricelistadmin/directive/VlocPriceListEntries.js":36,"./modules/pricelistadmin/directive/VlocPriceListHierarchy.js":37,"./modules/pricelistadmin/templates/templates.js":38}],3:[function(require,module,exports){
...
.controller('PriceListAdminController', ['$scope', '$rootScope', '$location', '$timeout', '$compile', 'remoteActions',
    function ($scope, $rootScope, $location, $timeout, $compile, remoteActions) {
        ...
        $scope.getObject = function(priceListId) {
            remoteActions.getObject(priceListId).then(function(results) {
                console.log('getObject results: ', results);
                $scope.price = results;
                $scope.editPrice = {};
                for (var key in $scope.price) {
                    $scope.editPrice[key] = $scope.price[key];
                }
            });
        };

        $scope.getContextCriteria = function(priceListId) {
            remoteActions.getContextCriteria(priceListId).then(function(results) {
                $scope.contextCriteria = results;
                $rootScope.$broadcast('setContextCriteria', $scope.contextCriteria);
            });
        };
        ...
        $scope.createItem = function(event) {
            ...
            var itemToSave = {};
            for (var key in $scope.editPrice) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = $scope.editPrice[key];
                }
            }

            remoteActions.createObject($scope.nsp + $scope.OBJECT_NAME, itemToSave).then(function(results) {
                console.log('create price list results: ', results);
                for (var key in results) {
                    $scope.price[key] = results[key];
                    $scope.editPrice[key] = results[key];
                }
                if (event) {
                    event.currentTarget.innerText = originalText;
                    event.currentTarget.disabled = false;
                ...
                window.location = '/apex/PriceListAdmin?id=' + results.Id + '&name=' + results.Name;
            }, function(error) {
                //TODO: show save failure message
                if (event) {
                    event.currentTarget.innerText = 'Error!';
        ...
        $scope.saveItem = function(event) {
            ...
            for (var key in $scope.editPrice) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = ($scope.editPrice[key] === null ? undefined : $scope.editPrice[key]);
                }
            }
            ...
            remoteActions.updateObject(itemToSave).then(function(results) {
                ...
                for (var key in results) {
                    if ($scope.objectFields[key] && $scope.objectFields[key].type === 'DATE') {
                        ...
                        $scope.editPrice[key] = results[key] + tzOffset;
                    } else {
                        $scope.price[key] = results[key];
                        $scope.editPrice[key] = results[key];
                    }
        ...
        $scope.openFacetDetails = function(facetType, facetData) {
            ...
            if (facetType === 'CONTEXT_CRITERIA_ITEM') {
                $scope.contextCriteriaItem = facetData.contextCriteriaItem;
                compiledHTML = $compile('<vloc-context-rule item="contextCriteriaItem" parent-item="price" mode="\'leftPane\'"></vloc-context-rule>')($scope);
                ...
                directiveHTML = '<vloc-pricing-element object-id="priceListId" price-list-entry-item="priceListEntryItem" pricing-element-object-type="pricingElementObjectType" parent-item="price" pricing-mode="pricingMode" is-adj="isAdj"></vloc-pricing-element>';
        ...
        $scope.init = function() {
            ...
            if ($scope.mode === 'new') {
                ...
                // $scope.getObjectLayoutById($scope.priceListId, true);
...
angular.module('pricelistadmin')
.directive('vlocPriceListEntries', ['remoteActions',
    function(remoteActions) {
        return {
            scope: {
                customViewAttrs: '='
            ...
            replace: true,
            restrict: 'E',
            templateUrl: 'PriceListEntries.tpl.html',
            controller: function($scope, $rootScope, $sldsModal, $compile) {
                console.log('INIT: ', $scope.customViewAttrs);
...
angular.module('pricelistadmin')
.directive('vlocPriceListHierarchy', ['remoteActions',
    function(remoteActions) {
        return {
            scope: {
                customViewAttrs: '='
            ...
            controller: function($scope, $rootScope, $sldsModal, $compile) {
                ...
                $scope.getPriceListHierarchy = function() {
                    remoteActions.getPriceListHierarchy($scope.priceListId).then(function(results) {
