...
angular.module('contractadmin', ['vlocity', 'ngSanitize', 'cpqdirectives', 'sldsangular'])
...
require('./modules/contractadmin/controller/ContractAdminController.js');

},{"./CpqDirectives.js":2,"./modules/contractadmin/controller/ContractAdminController.js":3}],2:[function(require,module,exports){
    ...
    function ($scope, $rootScope, $location, $timeout, $compile, remoteActions) {
        ...
        $scope.getObjectLayoutByName = function(objectName, recordType) {
            remoteActions.getObjectLayoutByName(objectName, recordType).then(function(results) {
                console.log('getObjectLayoutByName contract results: ', results);
        ...
        $scope.getObjectLayoutById = function(objectId, forSelf) {
            remoteActions.getObjectLayoutById(objectId, forSelf).then(function(results) {
                console.log('getObjectLayoutById contract results: ', results);
                $scope.buildObjectLayout(results);
            });
        };
        ...
        $scope.getObject = function(objectId) {
            remoteActions.getObject(objectId).then(function(results) {
                console.log('getObject results: ', results);
                $scope.contract = results;
                $scope.editContract = {};
                for (var key in $scope.contract) {
                    $scope.editContract[key] = $scope.contract[key];
                }
            });
        };
        ...
        $scope.createItem = function(event) {
            ...
            var itemToSave = {};
            for (var key in $scope.editContract) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = $scope.editContract[key];
                }
            }

            remoteActions.createObject($scope.nsp + $scope.OBJECT_NAME, itemToSave).then(function(results) {
                console.log('create contract results: ', results);
                for (var key in results) {
                    $scope.contract[key] = results[key];
                    $scope.editContract[key] = results[key];
                }
                if (event) {
                ...
                window.location = '/apex/ContractAdmin?id=' + results.Id + '&name=' + results.Name;
            }, function(error) {
        ...
        $scope.saveItem = function(event) {
            ...
            for (var key in $scope.editContract) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = ($scope.editContract[key] === null ? undefined : $scope.editContract[key]);
                }
            }

            remoteActions.updateObject(itemToSave).then(function(results) {
                console.log('save contract results: ', results);
                $scope.contract = {};
                $scope.editContract = {};
                for (var key in results) {
                    if ($scope.objectFields[key] && $scope.objectFields[key].type === 'DATE') {
                        ...
                        $scope.contract[key] = results[key] + tzOffset;
                        $scope.editContract[key] = results[key] + tzOffset;
                    } else {
                        $scope.contract[key] = results[key];
                        $scope.editContract[key] = results[key];
                    }
        ...
        $scope.openFacetDetails = function(facetType, facetData) {
            ...
            if (facetType === 'PROD_PRICING_COMPONENT') {
                $scope.productId = facetData.productId;
                $scope.offerComponentId = facetData.offerComponentId;
                $scope.contractId = facetData.contractId;
                $scope.offerPricingItem = facetData.offerPricingItem;
                $scope.mode = facetData.mode;
                console.log('SENDING PRODUCT ID 2: ', $scope.productId);
                directiveHTML = '<vloc-offer-pricing-component offer-id="productId" offer-component-id="offerComponentId" contract-id="contractId" ' +
                                'offer-pricing-item="offerPricingItem" mode="mode"></vloc-offer-pricing-component>';
        ...
        $scope.setupViewAttrs = function(section) {
            $scope.customViewData = {
                ...
                'attrs': {
                    'objectId': $scope.contractId,
                    'contractItems': $scope.contractItems
                }
            };
        };
        ...
        $scope.init = function() {
            ...
            if ($scope.mode === 'new') {
                $scope.contractName = 'New Contract';
                $scope.getObjectLayoutByName($scope.OBJECT_NAME, '');
            } else {
                $scope.getObjectLayoutById($scope.contractId, true);
                $scope.getObject($scope.contractId);
            }
        };
        $scope.init();
    }
