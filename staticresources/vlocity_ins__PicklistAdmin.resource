...
},{"./CpqDirectives.js":1,"./modules/picklistadmin/controller/PicklistAdminController.js":35,"./modules/picklistadmin/directive/VlocPicklistItem.js":36,"./modules/picklistadmin/directive/VlocPicklistItems.js":37,"./modules/picklistadmin/templates/templates.js":38}],3:[function(require,module,exports){
...
.controller('PicklistAdminController', ['$scope', '$rootScope', '$location', '$timeout', '$compile', 'remoteActions',
    function ($scope, $rootScope, $location, $timeout, $compile, remoteActions) {
        ...
        $scope.$on('refreshItems', function() {
            $scope.getPicklistValues($scope.picklistId);
        });
        ...
        $scope.getObject = function(picklistId) {
            remoteActions.getObject(picklistId).then(function(results) {
                ...
                for (var key in $scope.picklist) {
                    $scope.editPicklist[key] = $scope.picklist[key];
                }
            });
        };
        ...
        $scope.getPicklistValues = function(picklistId) {
            remoteActions.getPicklistValues(picklistId).then(function(results) {
        ...
        $scope.createItem = function(event) {
            ...
            var itemToSave = {};
            for (var key in $scope.editPicklist) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = $scope.editPicklist[key];
                }
            }

            remoteActions.createObject($scope.nsp + $scope.OBJECT_NAME, itemToSave).then(function(results) {
                console.log('create picklist results: ', results);
                for (var key in results) {
                    $scope.picklist[key] = results[key];
                    $scope.editPicklist[key] = results[key];
                }
                if (event) {
                    event.currentTarget.innerText = originalText;
                    event.currentTarget.disabled = false;
                ...
                window.location = '/apex/PicklistAdmin?id=' + results.Id + '&name=' + results.Name;
            }, function(error) {
                //TODO: show save failure message
                if (event) {
                    event.currentTarget.innerText = 'Error!';
        ...
        $scope.saveItem = function(event) {
            ...
            for (var key in $scope.editPicklist) {
                if (key !== '$$hashKey') {
                    itemToSave[key] = ($scope.editPicklist[key] === null ? undefined : $scope.editPicklist[key]);
                }
            }
            ...
            remoteActions.updateObject(itemToSave).then(function(results) {
                ...
                for (var key in results) {
                    if ($scope.objectFields[key] && $scope.objectFields[key].type === 'DATE') {
                        ...
                        $scope.editPicklist[key] = results[key] + tzOffset;
                    } else {
                        $scope.picklist[key] = results[key];
                        $scope.editPicklist[key] = results[key];
                    }
        ...
        $scope.init = function() {
            ...
            if ($scope.mode === 'new') {
                ...
                // $scope.getObjectLayoutById($scope.picklistId, true);
...
angular.module('picklistadmin')
.directive('vlocPicklistItem', ['remoteActions',
    function(remoteActions) {
        return {
            scope: {
                item: '=',
...
angular.module('picklistadmin')
.directive('vlocPicklistItems', ['remoteActions',
    function(remoteActions) {
        return {
            scope: {
                customViewAttrs: '='
            ...
            controller: function($scope, $rootScope, $sldsModal) {
                ...
                $scope.getFieldSetsByName = function(objectName) {
                    remoteActions.getFieldSetsByName(objectName).then(function(results) {
                        if (results[$scope.nsp + 'picklistitems'] === undefined) {
                            $scope.fieldSet = null;
                        } else {
                            $scope.fieldSet = results[$scope.nsp + 'picklistitems'];
                        }
